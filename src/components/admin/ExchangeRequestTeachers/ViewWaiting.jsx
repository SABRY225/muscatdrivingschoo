...
console.log("Waiting");
  ...
  function filterList(id) {
    setExchangeRequestsTeachers((pre) => pre.filter((item) => item.id !== id));
  }
  ...
  async function handleAccept(id) {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_KEY}api/v1/admin/exchangerequestteachersbystatus/${id}`,
        {
          method: "PUT",
          headers: {
            Authorization: token,
        ...
        <Paper sx={{ width: "100%", padding: "20px" }}>
          <TableContainer sx={{ maxHeight: 440 }}>
            ...
            <Table stickyHeader aria-label="sticky table">
              ...
              <TableBody>
                {ExchangeRequestsTeachers.length > 0 && checkFound
