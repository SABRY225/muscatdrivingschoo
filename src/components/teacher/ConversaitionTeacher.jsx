...
export default function ConversaitionTeacher({ chat }) {
    ...
    const fetchMessages = async () => {
      try {
        ...
        const res = await axios.get(
          `${process.env.REACT_APP_API_KEY}api/v1/chat/${chat?.id}/${teacher.id}`
        );
        res?.data?.messages.map(async(message) =>{
          if(message.receiverId === teacher.id){
            await axios.patch(
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      formData.append("senderId", teacher.id);
        ...
        {state.loading ? (
          ...
          <>
            {state.messages?.map((msg, index) => (
              <Message
                key={msg.id || index}
                sender={msg.senderId === teacher.id}
                message={msg}
              />
            ))}
