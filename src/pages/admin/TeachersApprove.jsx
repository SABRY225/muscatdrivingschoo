...
export default function TeachersApprove() {
  ...
  async function acceptTeacher(id) {
    ...
    const response = await fetch(
      `${process.env.REACT_APP_API_KEY}api/v1/admin/accept/${id}`,
      {
        method: "POST",
        headers: {
          Authorization: token,
    ...
    enqueueSnackbar("تم قبول المعلم", {
      variant: "success",
  ...
  async function rejectTeacher(id) {
    closeSnackbar();
    const response = await fetch(
      `${process.env.REACT_APP_API_KEY}api/v1/admin/reject/${id}`,
      {
        method: "POST",
        headers: {
          Authorization: token,
