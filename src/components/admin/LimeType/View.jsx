...
import { Box, Button, Dialog ,Avatar , Paper, TableBody , Table , TableRow , TablePagination , TableContainer , TableCell } from "@mui/material";
...
import LimeTypeUpdate   from "./Update";
...
export default function View() {
  ...
  const { data, isLoading } = useLimeType();
  const [LimeTypes, setLimeTypes] = useState([]);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(
        `${process.env.REACT_APP_API_KEY}api/v1/admin/limetype/${id}`,
        {
          method: "DELETE",
          headers: {
      ...
      if (res.ok) {
        ...
        setLimeTypes(LimeTypes.filter((c) => c.id !== id));
      } else {
