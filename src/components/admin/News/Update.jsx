...
export default function Update({handleClose,newsrow,setNewsData}) {
    const {t} = useTranslation()
    const {enqueueSnackbar} = useSnackbar()
    const lang = Cookies.get("i18next") || "en";
    const { register,control, formState: { errors }, handleSubmit } = useForm({
        defaultValues: {
            title_ar:newsrow?.titleAR,
            title_en:newsrow?.titleEN,
            descriptionAR : newsrow?.descriptionAR,
            descriptionEN : newsrow?.descriptionEN,
        }
    });
    ...
    formData.append("descriptionAR",    data.descriptionAR);
        try{
            const response = await fetch(`${process.env.REACT_APP_API_KEY}api/v1/admin/news/${newsrow.id}`,{
                method:"PUT",
                headers:{
                    "Authorization":token,
                },
            ...
            setNewsData(back=>back.map(item=>
                {
                    return item.id===newsrow.id?{...item,titleAR:data.title_ar,
