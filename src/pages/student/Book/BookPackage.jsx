...
export default function BookPackage() {
  ...
  const { data , isLoadingPackage}  = usePackage(packageId);
  const [ objPackage , setPackage]  = useState("");
  ...
  const {
    ...
    defaultValues: {
      typeLesson: "",   date: "",   typeofbook: "", price : objPackage?.price,
    },
  });
  ...
  async function onSubmit(data) {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_KEY}api/v1/payment/bookingPackage`,
        {
          method: "POST",
          headers: {
