...
export default function Update({handleClose,limetype,setLimeTypes}) {
    const {t} = useTranslation()
    const {enqueueSnackbar} = useSnackbar()
    const lang = Cookies.get("i18next") || "en";
    const { register,control, formState: { errors }, handleSubmit } = useForm({
        defaultValues: {
            title_ar:limetype?.titleAR,
            title_en:limetype?.titleEN
        }
    });
    ...
    async function onSubmit(data)
    {
        ...
        try{
            const response = await fetch(`${process.env.REACT_APP_API_KEY}api/v1/admin/updateLimeType/${limetype.id}`,{
                method:"PUT",
                headers:{
                    "Authorization":token
                },
            ...
            setLimeTypes(back=>back.map(item=>
                {
                    return item.id===limetype.id?{...item,titleAR:data.title_ar,titleEN:data.title_en}:item
                }))
            handleClose()
        }
        catch(err)
