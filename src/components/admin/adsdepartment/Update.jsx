...
export default function Update({handleClose,adsdepartment,setAdsDepartments}) {
    const {t} = useTranslation()
    const {enqueueSnackbar} = useSnackbar()
    const lang = Cookies.get("i18next") || "en";
    const { register,control, formState: { errors }, handleSubmit } = useForm({
        defaultValues: {
            title_ar:adsdepartment?.titleAR,
            title_en:adsdepartment?.titleEN
        }
    });
    ...
    formData.append("titleEN",          data.title_en);
        ...
        try{
            const response = await fetch(`${process.env.REACT_APP_API_KEY}api/v1/admin/adsdepartment/${adsdepartment.id}`,{
                method:"PUT",
                headers:{
                    "Authorization":token,
                },
            ...
            setAdsDepartments(back=>back.map(item=>
                {
                    return item.id===adsdepartment.id?{...item,titleAR:data.title_ar,titleEN:data.title_en}:item
                }))
            handleClose()
        }
        catch(err)
