...
export default function TrainingCategoryTypesUpdate({handleClose,trainingcategorytype,setTrainingCategoryTypes}) {
    const {t} = useTranslation()
    const {enqueueSnackbar} = useSnackbar()
    const lang = Cookies.get("i18next") || "en";
    const { register,control, formState: { errors }, handleSubmit } = useForm({
        defaultValues: {
            title_ar:trainingcategorytype?.titleAR,
            title_en:trainingcategorytype?.titleEN
        }
    });
    ...
    formData.append("titleEN",          data.title_en);
        ...
        try{
            const response = await fetch(`${process.env.REACT_APP_API_KEY}api/v1/admin/updateTrainingCategoryType/${trainingcategorytype.id}`,{
                method:"PUT",
                headers:{
                    "Authorization":token,
                },
            ...
            setTrainingCategoryTypes(back=>back.map(item=>
                {
                    return item.id===trainingcategorytype.id?{...item,titleAR:data.title_ar,titleEN:data.title_en}:item
                }))
            handleClose()
        }
        catch(err)
