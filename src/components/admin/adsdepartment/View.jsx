...
import { Box, Button, Dialog , Paper , Table, TableBody , TableCell, TableContainer ,TablePagination , TableRow  } from "@mui/material";
...
import AdsDepartmentUpdate       from "./Update";

export default function View() {
  const { t } = useTranslation();

  const columns = [
    { id: "nameAR",      label: t("titleAr"), minWidth: 150 },
    { id: "nameEN",      label: t("titleEn"), minWidth: 150 },
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(
        `${process.env.REACT_APP_API_KEY}api/v1/admin/adsdepartment/${id}`,
        {
          method: "DELETE",
          headers: {
      ...
      if (res.ok) {
        ...
        setAdsDepartments(AdsDepartments.filter((c) => c.id !== id));
      } else {
                      ...
                      <TableRow hover role="checkbox" key={row.id + "demj"}>
                        ...
                        <TableCell align="center">
                          ...
                          <Dialog open={open === row.id} onClose={handleClose}>
                            <AdsDepartmentUpdate
                              setAdsDepartments={setAdsDepartments}
                              adsdepartment={row}
