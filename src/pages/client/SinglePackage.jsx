        ...
        const fetchTeacher = async () => {
            try {
                ...
                const res2 = await axios.get(
                    `${process.env.REACT_APP_API_KEY}api/v1/teacher/getSinglePackage/${packageId}`
                );
                setTeacherData(res.data);
                setPackageData(res2.data.data);
            } catch (error) {
    ...
    return (
        <Navbar>
            <Container sx={{ marginBottom: "40px", marginTop: "80px" }}>
                {packageData?<Grid container spacing={3}>
                    <Grid item xs={12} md={12} lg={7}>
                        {packageData && <AboutPackage packageData={packageData} />}
                    </Grid>
