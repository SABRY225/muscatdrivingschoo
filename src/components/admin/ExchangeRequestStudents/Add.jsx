...
export default function Add() {
    ...
    const { data:studetns , isLoadingTeacher  }     = useStudents();
    const [teachersTypeError ] = useState("");
    const [StudentId , setTeacherValue ] = useState("");
          ...
          <Box sx={{ marginBottom: "26px" }}>
              ...
              <Autocomplete
               onChange={(event, newValue) => {
                setTeacherValue(newValue?.id || null);
               }}
                fullWidth
                name    ="student"
                options={studetns?.data}
                getOptionLabel={(op) =>
                ...
                renderInput={(params) => (
                  <TextField
                    {...params}
                    label={lang === "en" ? "Choose a Students " : "إختر الطلاب"}
                    inputProps={{
                      ...params.inputProps,
